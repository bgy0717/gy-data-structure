...
void main(){
    ...
    fill_data(matrix_a, row, col); //matrix_a 값 채우기
    fill_data(matrix_b, row, col); //matrix_b 값 채우기
    printf("<matrix_a>\n");
    ...
    free_matrix(matrix_axt, small, small); //모든 행렬의 메모리 반납
...
int multiply_matrix(int** matrix_a, int** matrix_t, int ** matrix_axt, int row, int col){ //행렬 곱셈함수
    ...
    for(m=0; m<small; m++){
        for(n=0; n<small; n++){
            matrix_axt[m][n] = 0;    //axt의 값들을 0으로 초기화
            for(i=0; i<big; i++){
                matrix_axt[m][n] += matrix_a[m][i]*matrix_t[i][n]; //axt 행렬의 값 계산
            }
        }
    }
